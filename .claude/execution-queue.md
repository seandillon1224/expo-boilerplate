...
- [B] **#26 T2.7** — Renovate auto-merge wiring (required checks, branch protection as code via `gh` script). _Blocked: applying live branch protection was denied by the permission classifier; needs the user to approve (or run `--apply` themselves)._
