...
- [x] **#145 T5.8** — Adopt EAS built-ins: `update-rollout` in promote, `testflight` in release, `eas/send_slack_message` for the Slack jobs (audit 2026-09-13; after #66).
...
- [x] **#56 T7.5** — End-to-end test: spawn a throwaway project from the template in CI and run the JS gate on it.
...
- [x] **#59 T8.3** — ADR folder with the decisions table above as ADR-0001.
- [x] **#143 T8.4** — Docs site: VitePress over `docs/`, GitHub Pages deploy, `Docs` CI check, landing + sidebar + link audit (decided 2026-09-13).

### E9 — Deferred deep-dive research tickets (tracker #76)

- [x] **#60 T9.1** — D1 release-please + versioning.
- [x] **#61 T9.2** — D2 multi-runtime OTA backports (grilled 2026-09-13: manual `backport.yml`, tags + fix SHA inputs, fingerprint-equality gate, approval + rollout straight to production, prepared-branch path for conflicts; ADR-0008).
...
- [x] **#63 T9.4** — D4 Flashlight (decided 2026-09-13: informational `after_maestro_tests` hook in the Android maestro job, opt-in constant, artifact only, no gate; ADR-0007).
