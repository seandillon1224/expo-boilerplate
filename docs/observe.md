...
| `no EAS session … skipping`                                   | Not logged in and no `EXPO_TOKEN`. Locally `bun run eas login`; on EAS the workflow exports the run's robot token; on GitHub Actions the `EXPO_TOKEN` repository secret ([owner checklist](owner-checklist.md#github-repository-secrets)).                                                                      |
