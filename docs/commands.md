...
| Command                           | Does                                                                                                                                                                                                                                                                           |
| --------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `bun run e2e:web`                 | Maestro `web`-tagged flows (`.maestro/flows/web/`) against the static export; needs `bun run export:web:e2e` and `bun run serve:web` running first                                                                                                                             |
| `bun run export:web:e2e`          | `export:web` with `EXPO_PUBLIC_API_URL` baked to the local fixture API (`http://localhost:8081/fixtures`, override with `E2E_API_URL`), so the web flows read `.maestro/fixtures/posts.json` instead of jsonplaceholder. What CI's `Bundle budget (web)` job exports           |
