...
| `EXPO_PUBLIC_UPDATE_POLICY` | `plaintext` | `preview` (`forced`), `production` (`silent`) | **Owner** (optional)       | `useUpdatePolicy` (`src/features/updates`): `silent` (default when unset) \| `opt-in` \| `forced`; see [Release ladder → Update policies](release-ladder.md#update-policies).           |
...
`staging` from `uat` inside the shared `preview` environment. `EAS_UPDATE_CRITICAL` is not one
either: it is set on a single `eas update` (the workflows' `critical` input, #137) so that
`app.config.ts` marks that one update forced; as an environment variable every update would be. Add new `EXPO_PUBLIC_*` keys to
