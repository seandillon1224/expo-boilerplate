...
| -------------------- | --------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------- |
| **PR**               | open / push a PR into `main`                              | JS gate (`ci.yml`), native Maestro check (`e2e.yml`), web preview to the `pr-<number>` alias, fingerprint-drift comment | Required checks + review ([JS gate](js-gate.md)); nothing on the ladder moves              | [PR previews](#pr-previews-web-opt-in), [Drift](#fingerprint-drift-on-prs-informational) |
...
## PR previews (web, opt-in)

**Workflow:** `.eas/workflows/preview-web.yml` (`Preview web`). **Trigger:** adding the
`web-preview` label to a PR (`on: pull_request_labeled`; fork PRs never trigger EAS workflows),
plus `workflow_dispatch`. It is label-gated rather than every-PR so that the checks list carries
no permanent "skipped" `Preview web` entry while `HOSTING` is disabled (T11.6); a later push does
**not** re-trigger it — remove and re-add the label for a fresh deploy. Once `HOSTING` is enabled
and previews are worth paying for on every PR, swap the trigger back to
