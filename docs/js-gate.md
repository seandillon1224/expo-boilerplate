...
| `Bundle budget (web)`     | `CI` / `bundle-budget-web` | `bun run export:web:e2e && bun run budget --platform web` (the fixture-API export the `Maestro web` job reuses)                              | PR, push to main | yes      | `bundle-sizes-web`, `web-export`  | ~75 s                                   |
...
| Maestro web       | `bun run export:web:e2e && bun run serve:web` in one shell, `bun run e2e:web` in another                                         | Needs Maestro ≥ 2.9 (`curl -Ls https://get.maestro.mobile.dev \| bash`) and a JDK; report in `maestro-web/`. |
